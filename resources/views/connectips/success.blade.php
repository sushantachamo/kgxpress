            ...
            <div class="container">
                <div class="row cols-xs-space cols-sm-space cols-md-space">
                    <div class="col-lg-8">
                        ...
                        <form action="{{ route('payment.checkout') }}" class="form-default" data-toggle="validator" role="form" method="POST" id="checkout-form">                            @csrf
                            <div class="card">
                                <div class="card-title px-4 py-3">
                                    <h3 class="heading heading-5 strong-500">
                                        @foreach ($resp as $key => $value)
                                            @if ($key == "status" && $value == "SUCCESS")
                                                {{ translate('Payment Successfull')}}
                                            @endif
                                        ...
                                        <div class="col-md-12">
                                            <div class="row">
                                                <div class="col-lg text-left font-weight-normal text-capitalize">
                                                    @foreach ($resp as $key => $value)
                                                        @if ($key == "txnAmt" || $key == "status" || $key == "statusDesc")
                                                            @if ($key == "txnAmt")
                                                                <h4>
                                                                    {{$key}} :
                                                                    <small class="text-muted">{{$value /100}}</small>
                                                                </h4>
                                                            @else
                                                                <h4>
                                                                    {{$key}} :
                                                ...
                                                <div class="col-lg d-flex justify-content-center">
                                                    <div class="block-icon c-gray-light mb-0">
                                                        @foreach ($resp as $key => $value)
                                                            @if ($key == "status" && $value == "SUCCESS")
                                                                <i class="la la-check-circle" style="color:#72BF40;font-size:7em"></i>
                                                            @endif
                                                            @if ($key == "status" && $value == "FAILED")
