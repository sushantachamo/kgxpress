		...
		<table class="padding text-left small border-bottom">
			<thead>
                <tr class="gry-color" style="background: #eceff4;">
                    <th width="50%">{{translate('Sub-Sub-Category Name')}}</th>
                    <th width="30%">{{translate('Sub-Sub-Category ID')}}</th>
					<th width="20%">{{translate('Sub-Category ID')}}</th>
                </tr>
			</thead>
			<tbody class="strong">
                @foreach ($sub_sub_categories as $key => $sub_sub_category)
	                <tr class="">
						...
						<td>{{ $sub_sub_category->sub_category_id }}</td>
					</tr>
				@endforeach
            </tbody>
		</table>
